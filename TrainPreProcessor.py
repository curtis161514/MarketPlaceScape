...
def trainpreprocess(datafile,keyfile):
	...
	#combine any new data and drop duplicates
	data = pd.concat([olddata,newdata]).drop_duplicates()  

	#save data to csv for use nexttime
	data.to_csv('data1', header = True, index = False)
	...
	for add in range(num_adds):
		...
		print('pre-processing ',add+1,' of ',num_adds,' valid listings in database')
